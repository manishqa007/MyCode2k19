...
public class Matrix02_PrintSpiralMatrix {
	public static void main(String[] args) {
		...
		row_index_start = 0;	row_index_end = 3;
		...
		for (j = row_index_start; j <= row_index_end; j++) { // Printing 1st column
			System.out.print(arr[j][col_index_end] + " ");
		}
		col_index_end--;  //col_index_end = 3
		
		for(j= col_index_end ;j>=col_index_start; j--) {
			System.out.print(arr[row_index_end][j]+" ");
		}
		row_index_end--; // row_index_end = 3
		
		for(j= row_index_end;j>row_index_start; j--) { 
			System.out.print(arr[row_index_end][col_index_start]+" ");
		}
		col_index_start--;
	}
}
